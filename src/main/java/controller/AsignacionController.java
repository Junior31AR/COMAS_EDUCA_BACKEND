...
import service.RespuestaAsignacionService;
...
public class AsignacionController {
	...
	public ResponseEntity<?> crearAsignacion(@Valid @RequestBody Asignacion asignacion, BindingResult results,
		...
		if (results.hasErrors()) {
			List<String> errors = results.getFieldErrors().stream()
	...
	public ResponseEntity<?> actualizarAsignacion(@Valid @RequestBody Asignacion asignacion, BindingResult results,
		...
		if (asignacionEncontrada == null) {
			response.put("mensaje", "Error: No se pudo editar la clase con el ID: "
					.concat(idAsignacion.toString().concat(" no existe en la base de datos.")));
	...
	public ResponseEntity<?> getAsignacion(@PathVariable Long id) {
		...
		if (asignacion.isEmpty()) {
			response.put("mensaje",
					"La asignación con el ID: ".concat(id.toString().concat(" no existe en la base de datos")));
	...
	public ResponseEntity<?> actualizarRespuestaAsignacion(@RequestParam("nota") String nota,
			@PathVariable Long idRespuesta) {

		RespuestaAsignacion respuestaAsignacionEncontrada = respuestaAsignacionService.findById(idRespuesta)
				.orElse(null);
	...
	public ResponseEntity<?> crearRespuestaAsignacion(@RequestParam("idAsignacion") String idAsignacion,
			@RequestParam("archivo") MultipartFile archivo, @RequestParam("dniEstudiante") String dniEstudiante) {
		...
		if (!archivo.isEmpty()) {
			...
			try {
				...
				respuestaAsignacion.setNombresEstudiante(estudiante.getNombres() + " " + estudiante.getApellidoPaterno()
						+ " " + estudiante.getApellidoMaterno());
				respuestaAsignacion.setDniEstudiante(dniEstudiante);
				respuestaAsignacionNueva = respuestaAsignacionService.save(respuestaAsignacion,
						Long.parseLong(idAsignacion));
			} catch (IOException e) {
	...
	public ResponseEntity<?> buscarRespuestaEstudiante(@RequestParam("dniEstudiante") String dniEstudiante,
