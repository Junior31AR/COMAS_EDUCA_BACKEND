...
//	@PreAuthorize("hasRole('ADMIN')")
	...
	public ResponseEntity<?> getEstudiantePorUserAndPassword(@RequestParam String username,
		...
		if (estudiante == null) {
			response.put("mensaje",
					"El estudiante con el DNI: ".concat(username.toString().concat(" no existe en la base de datos")));
	...
	@PreAuthorize("hasRole('ADMIN')")
	@PutMapping("/{id}")
	public ResponseEntity<?> updateEstudiante(@Valid @RequestBody Estudiante estudiante, BindingResult results,
...
//			estudianteActual.setUsuario(estudiante.getUsuario());
	...
	public ResponseEntity<?> updateEstudianteAula(@Valid @RequestBody Estudiante estudiante, BindingResult results,
			@PathVariable Long id) {
		Estudiante estudianteActual = estudianteService.getEstudianteById(id).get();
		Estudiante estudianteActualizado = null;
		Map<String, Object> response = new HashMap<>();
	...
	public ResponseEntity<?> deleteEstudiante(@PathVariable Long id) {
		...
		if (estudiante == null) {
			response.put("mensaje", "Error: No se pudo eliminar, el estudiante con el ID: "
					.concat(id.toString().concat(" no existe en la base de datos")));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.NOT_FOUND);
		}
