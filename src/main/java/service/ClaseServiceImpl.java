...
public class ClaseServiceImpl implements ClaseService {
	...
	public byte[] generarReporteCurso(String tipo, Long idCurso, Long idGrado) {
		...
		if (listacursoReporte == null)
			return data;

		try {
			File file = new File(getClass().getClassLoader().getResource("cursoReporte.jasper").getFile());

			JasperPrint rpt = JasperFillManager.fillReport(file.getPath(), null,
					new JRBeanCollectionDataSource(listacursoReporte));
	...
	> asignacionesPorClase(Long idClase) {
	...
	private List<CursoReporte> buscarAprobadosYDesaprobados(List<Aula> aulas, Long idCurso) {
		...
		for (Aula aula : aulas) {
			...
			if (clase != null) {
				...
				CursoReporte cursoReporte = new CursoReporte(
						notas.get(0).getEstudiante().getAulaEstudiante().getNombre()
								+ notas.get(0).getEstudiante().getAulaEstudiante().getSeccion(),
						notas.get(0).getCurso().getNombre(), aprobados, desaprobados);
